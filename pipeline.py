...
def pipelineMain():
	...
	p.Instruction_Memory.Store_Word(8,  0x00000000) # nop
	p.Instruction_Memory.Store_Word(12, 0x11090004) # beq t0 t1 0x0004
	...
	p.Instruction_Memory.Store_Word(32, 0x200a0009) # addi t2 zero 0x0009
...
class Pipeline(object):
	...
	def ID(self):
		...
		# branch hazards
		# if (((self.EXMEM.memControl_out.MemRead) and 
		...
		# (((self.IDEX.memControl_out.MemRead) &((self.IDEX.WbReg== IF/ID.Reg.Rt) |(ID/EX.WbReg== IF/ID.Reg.Rs)))):
		...
		self.IDEX.set(self.IFID.instruction_out, sign_extend_imm, self.IFID.pc_out, read_data_1, 
			read_data_2, branch_addr_temp, wbcTemp, memcTemp, excTemp)

	def EX(self):
		# determine write reg address
		dest_reg_temp = HW.Register_Input_Mux(self.IDEX.instruction_out.rt, 
			self.IDEX.instruction_out.rd, self.IDEX.exControl_out.RegDst)
		...
		self.EXMEM.set(self.IDEX.instruction_out, dest_reg_temp, self.IDEX.readData2_out, alu_res_temp, 
			(alu_res_temp == 0), jump_addr_temp, self.IDEX.wbControl_out, 
